    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            if namespace_btn.count() > 0:
                ...
                # The options are already visible in the popup list, so we can click them directly in one go
                for option in ["green", "observe-green", "blue"]:
                    print(f"   Selecting '{option}'...")
                    try:
                        # Find exact text in the UI list. The popover is appended to body, so last() usually hits the list element.
                        option_el = page.get_by_text(option, exact=True).last
                        option_el.scroll_into_view_if_needed()
